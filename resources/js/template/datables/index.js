...
module.exports = function(h, modulesTemp, classes, slots) {
  const modules = {
    ...
    dropdownPaginationCount: require('./modules/dropdown-pagination-count').call(
      this,
      h
    ),
  ...
  const genericFilter = this.hasGenericFilter
    ? h('div', { class: 'VueTables__search-field el-input--small' }, [
        h(
          'label',
          {
            attrs: { for: filterId },
  ...
  const perpage =
    perpageValues.length > 1
      ? h('div', { class: 'VueTables__limit-field el-input--small' }, [
          h('label', { class: classesCs.label, attrs: { for: perpageId } }, [
            this.display('limit'),
          ]),
  ...
  const footerHeadings = this.opts.footerHeadings
    ? h('tfoot', [h('tr', [modules.headings(classesCs.right)])])
    : '';
  ...
  const tableTop =
    ...
    slots.beforeLimit ||
    slots.afterLimit
      ? h('div', { class: classesCs.row }, [
          h('div', { class: classesCs.column }, [
            h(
              'div',
              {
  ...
  return h('div', { class: `VueTables VueTables--${this.source}` }, [
      ...
      h(
        'table',
        {
          class: `VueTables__table ${
            this.opts.skin ? this.opts.skin : classesCs.table
          }`,
        },
        [
          h('thead', [
            h('tr', [modules.headings(classesCs.right)]),
            slots.beforeFilters,
            modules.columnFilters(classesCs),
            slots.afterFilters,
          ]),
          footerHeadings,
          slots.beforeBody,
    ...
    slots.afterTable,
    modules.pagination(
      (0, _merge2.default)(classesCs.pagination, {
        wrapper: `${classesCs.row} ${classesCs.column} ${classesCs.contentCenter}`,
        nav:
          classesCs.center + ' el-pagination is-background mt-6 font-hairline',
