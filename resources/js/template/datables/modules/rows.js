...
module.exports = function(h) {
  ...
  return function(classes) {
    ...
    if (_this.count === 0) {
      ...
      return h('tr', { class: 'VueTables__no-results' }, [
        h(
          'td',
          {
            class: 'text-center',
    ...
    data.map(function(row, index) {
      if (
        ...
        groupByContent = _this.opts.toggleGroups
          ? h(
              ...
              [
                groupValue,
                h('span', { class: _this.groupToggleIcon(groupValue) }),
              ]
            )
          : groupValue;
      ...
      if (
        _this.opts.toggleGroups &&
        _this.collapsedGroups.includes(currentGroup)
      ) {
        return;
      }
        ...
        var childRowToggler = h('td', [
          h('span', {
            ...
            class:
              ...
              _this.childRowTogglerClass(row[rowKey]),
          }),
        ]);
      ...
      _this.allColumns.map(function(column) {
        ...
        columns.push(
          h('td', { class: _this.columnClass(column) + ' px-4' }, [
            rowTemplate
              ? rowTemplate({ row: row, column: column, index: index })
      ...
      rowClass = _this.opts.rowClassCallback
        ? _this.opts.rowClassCallback(row)
        : '';
      ...
      rows.push(
        _this.hasChildRow && _this.openChildRows.includes(row[rowKey])
          ? h('tr', { class: 'VueTables__child-row' }, [
              h(
                'td',
                {
                  attrs: { colspan: _this.colspan },
