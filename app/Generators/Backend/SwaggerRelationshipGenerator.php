...
Class SwaggerRelationshipGenerator extends BaseGenerator
{
    ...
    private function generate($relationship, $model, $modelCurrent)
    {
        ...
        if ($relationship === $this->relationship['has_one'] || $relationship === $this->relationship['has_many']) {
            $templateData = $this->serviceGenerator->replaceNotDelete($this->notDelete['property'], $this->generateField(\Str::snake($model) . '_id', $relationship), 1, $templateData);
        } else {
            ...
            $templateData = $this->serviceGenerator->replaceNotDelete($this->notDelete['property'], $this->generateField(\Str::snake($model) . '_id', $relationship), 1, $templateData);
            $templateData = $this->serviceGenerator->replaceNotDelete($this->notDelete['property'], $this->generateField(\Str::snake($modelCurrent) . '_id', $relationship), 1, $templateData);
