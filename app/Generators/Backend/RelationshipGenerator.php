...
class RelationshipGenerator extends BaseGenerator
{
    ...
    private function _generateAddApi($model, $modelRelationship, $templateDataReal, $notDelete)
    {
        ...
        $templateDataReal = str_replace(
            $mountedStub,
            $mountedStub . $this->serviceGenerator->infy_nl_tab(1, 2, 2) . $stubGetData,
            $templateDataReal,
        );
        $templateDataReal = $this->serviceGenerator->replaceNotDelete(
            $notDelete['data'],
        ...
        if (!stripos($templateDataReal, $importStub)) {
            ...
            $templateDataReal = str_replace(
                $stubRCurrent,
                $stubRCurrent . $this->serviceGenerator->infy_nl_tab(1, 0) . $stubRelationship,
                $templateDataReal,
            );
        }
        return $templateDataReal;
    ...
    private function _generateIndexFe($modelRelationship, $columnRelationship, $options, $funcName, $relationship)
    {
        ...
        if (in_array($configOptions['show'], $options)) {
            ...
            $fieldsGenerateHeadings[] =
                "'" . $nameRelationship . "'" . ': () => this.$t(\'route.' . $funcName . '\')' . ',';
            ...
            $templateDataReal = str_replace(
                ...
                "$headings" . $this->replaceTemplateHeading($fieldsGenerateHeadings, 3) . '},',
                $templateDataReal,
            );
            //sort
        ...
        foreach ($options as $option) {
            if ($option === $configOptions['sort']) {
                ...
                if ($relationship === $this->relationship['has_one']) {
                    ...
                    $funcRelationship =
                        $this->serviceGenerator->modelNamePluralFe($modelRelationship) . '.' . $columnRelationship;
                }
                $templateDataReal = str_replace(
                    "$templateColumns]",
                    "$templateColumns" . "$commaColumns" . "$columnDidGenerate]",
                ...
                $templateDataReal = str_replace(
                    "$templateColumnWith]",
                    "$templateColumnWith" . "$commaColumnWith" . "$withRelationship]",
                    $templateDataReal,
                );
                ...
                $templateDataReal = str_replace(
                    ...
                    "$templateColumnSearch" . "$commaSearch" . "$columnDidGenerate]",
                    $templateDataReal,
                );
            }
        }
    ...
    private function _generateController($modelRelationship, $model, $options, $column, $relationship)
    {
        ...
        if ($relationship === $this->relationship['belongs_to_many']) {
            ...
            $templateCreateUpdate = str_replace(
                ...
                $this->serviceGenerator->tableNameNotPlural($modelRelationship) . self::_ID,
