...
class RelationshipGenerator extends BaseGenerator
{
    ...
    private function generate(
        ...
        $pathTemplate = 'Models/';
        $template = $this->serviceGenerator->get_template(
            "relationship",
            $pathTemplate
        );
        // Model Relationship
        ...
        $templateModelReal = $this->serviceGenerator->getFile(
            'model',
            'laravel',
            $model . '.php'
        );
        ...
        $templateModelCurrentReal = $this->serviceGenerator->getFile(
            'model',
            'laravel',
            $modelCurrent . '.php'
        );
        $this->_replaceFile(
            $modelCurrent,
            $templateModel,
            $templateModelCurrentReal
        );
    }

    private function _migrateRelationship(
        $relationship,
        $model,
        $modelCurrent,
        $column,
        ...
        if ($relationship === $this->relationship['belongs_to_many']) {
            //belongsToMany
            $templateData = $this->serviceGenerator->get_template(
                "migrationRelationshipMTM",
                $pathTemplate
            );
            //if belongsToMany replace table to create
            $templateData = $this->_replaceTemplateRelationshipMTM(
                $model,
                $modelCurrent,
                $templateData
            );
            ...
            if (
                $this->serviceGenerator->getOptions(
                    ...
                    $this->tableCurrent['options']
                )
            ) {
                $this->_generateIndexFe(
                    $modelCurrent,
                    $column2,
                    $options,
                    \Str::snake($model),
            ...
            //hasOne or hasMany
            $templateData = $this->_replaceTemplateRelationship(
                $model,
                $modelCurrent,
                $templateData
            );
    ...
    private function _generateFormFe(
        $model,
        $modelRelationship,
        $columnRelationship,
        $relationship
        ...
        $templateDataReal = str_replace(
            "$dataForm {" .
                $this->serviceGenerator->infy_nl_tab(1, 0) .
                $templateDataForm .
                "},",
            "$dataForm {" .
                $this->replaceTemplate($fieldsGenerateDataForm, 2, 3, 2) .
                "},",
            $templateDataReal
        );
        //create form item
        ...
        if ($relationship !== $this->relationship['belongs_to_many']) {
            ...
            $templateRules = str_replace(
                '{{$ATTRIBUTE_FIELD$}}',
                'this.$t(\'route.' . \Str::snake($modelRelationship) . '\')',
                $templateRules
            );
            $templateDataReal = $this->serviceGenerator->replaceNotDelete(
                $notDelete['rules'],
    ...
    private function _generateAddApi(
        $model,
        $modelRelationship,
        $templateDataReal,
        $notDelete
        ...
        $mountedStub = 'mounted() {';
        $stubGetData = $this->serviceGenerator->get_template(
            "getDataRelationship",
            'Handler/',
            'vuejs'
        );
        ...
        if (!stripos($templateDataReal, $importStub)) {
            ...
            $stubRCurrent =
                "const " .
                \Str::camel($modelRelationship) .
                "Resource" .
                " = new " .
                $this->serviceGenerator->modelNameNotPlural(
                    $modelRelationship
                ) .
                "Resource();";
            $templateDataReal = str_replace(
    ...
    private function _generateApi($model)
    {
        ...
        $stubAPI = $this->serviceGenerator->get_template(
            "apiRelationship",
            'Api/',
            'vuejs'
        );
        ...
        $this->serviceFile->createFile(
            config('generator.path.vuejs.api'),
            $fileName,
            $templateDataReal
        );
        return $templateDataReal;
    ...
    private function _generateIndexFe(
        $modelRelationship,
        $columnRelationship,
        $options,
        $funcName,
        ...
        if (in_array($configOptions['show'], $options)) {
            ...
            $templateDataReal = str_replace(
                ...
                "$columns" . implode(', ', $fieldsGenerateColumns) . "]",
                $templateDataReal
            );
            //headings
            ...
            if ($relationship === $this->relationship['belongs_to_many']) {
                ...
                $templateTag = $this->serviceGenerator->get_template(
                    "tagRelationshipMTM",
                    $pathTemplate,
                    'vuejs'
                );
                $fileNameTag = $funcName . '.' . $columnRelationship;
                $templateTag = str_replace(
                    '{{$FIELD_NAME$}}',
                    $fileNameTag,
                    $templateTag
                );
            ...
            $templateDataReal = str_replace(
                ...
                "$headings" .
                    $this->replaceTemplateHeading($fieldsGenerateHeadings, 3) .
                    "},",
                $templateDataReal
            );
            //sort
            if (in_array($configOptions['sort'], $options)) {
                ...
                $templateDataReal = str_replace(
                    "$sortable" . $templateSortable . "]",
                    "$sortable" . implode(', ', $fieldsGenerateSort) . "]",
                    $templateDataReal
                );
            }
            $fileName = config('generator.path.vuejs.views') . $fileName;
    ...
    private function _generateIndexTableFe(
        $modelRelationship,
        $columnRelationship,
        $options,
        $funcName,
    ...
    private function _generateOptions(
        ...
        foreach ($options as $option) {
            if ($option === $configOptions['sort']) {
                ...
                $commaColumns = ',';
                if (
                    \Str::endsWith($templateColumns, ',') ||
                    strlen($templateColumns) === strlen($columns)
                ) {
                    $commaColumns = '';
                }
                $columnDidGenerate =
                    '"' . \Str::snake($modelRelationship) . self::_ID . '"';
                $templateDataReal = str_replace(
                    "$templateColumns]",
                    "$templateColumns" .
                        "$commaColumns" .
                ...
                if ($relationship === $this->relationship['has_one']) {
                    $funcRelationship =
                        \Str::snake($modelRelationship) .
                        '.' .
                        $columnRelationship;
                } else {
                    $funcRelationship =
                        $this->serviceGenerator->modelNamePluralFe(
                            $modelRelationship
                        ) .
                        '.' .
                        $columnRelationship;
                ...
                if (strlen($templateColumnWith) === strlen($columnWith)) {
                    ...
                    $templateDataReal = str_replace(
                        "$templateColumnWith]",
                        "$templateColumnWith" .
                            $this->serviceGenerator->infy_nl_tab(0, 1) .
                            $sortRelationship .
                            $this->serviceGenerator->infy_nl_tab(1, 3) .
                            "]",
                        $templateDataReal
                ...
                $commaColumnWith = ',';
                if (
                    \Str::endsWith($templateColumnWith, ',') ||
                    strlen($templateColumnWith) === strlen($columnsWith)
                ) {
                    $commaColumnWith = '';
                }
                if (
                    ...
                    $withRelationship =
                        '"' .
                        $this->serviceGenerator->modelNameNotPluralFe(
                            $modelRelationship
                        ) .
                        '"';
                } else {
                ...
                if (
                    ...
                    strlen($templateColumnSearch) === strlen($columnSearch)
                ) {
                    $commaSearch = '';
                }
                if ($relationship === $this->relationship['belongs_to_many']) {
    ...
    private function _generateController(
        ...
        //if belongs_to_many
        if ($relationship === $this->relationship['belongs_to_many']) {
            $templateCreateUpdate = $this->serviceGenerator->get_template(
                "createUpdateRelationship",
                $pathTemplate
            );
            //replace create or update
            $paramCreateUpdateStub =
                $this->serviceGenerator->modelNameNotPluralFe(
                    $modelRelationship
                ) . 'Id';
            ...
            //replace show
            $templateShow = $this->serviceGenerator->get_template(
                "showRelationship",
                $pathTemplate
            );
            ...
            //replace delete
            $templateDelete = $this->serviceGenerator->get_template(
                "deleteRelationship",
                $pathTemplate
            );
        ...
        //generate options
        $templateDataReal = $this->_generateOptions(
            $modelRelationship,
            $options,
            $templateDataReal,
            $column,
        ...
        $templateDataRealFunc = $this->serviceGenerator->getFile(
            'api_controller',
            'laravel',
            $fileNameFunc
        );
        if (!stripos($templateDataRealFunc, 'get' . $modelRelationship)) {
            $templateDataFunc = $this->serviceGenerator->get_template(
                "relationship",
                $pathTemplate
            );
    ...
    private function _generateRoute($modelRelationship)
    {
        ...
        if (!stripos($templateDataReal, 'get' . $modelRelationship)) {
            $stubResource =
                "Route::apiResource('{{RESOURCE}}', '{{MODEL_CLASS}}Controller');";
            $stubRoute =
                "Route::get('/{{MODEL}}/get-{{MODEL_RELATIONSHIP}}', '{{CONTROLLER}}Controller@get{{ACTION}}');";
    ...
    private function _replaceTemplateRelationship(
        $model,
        $modelDif,
        $templateData
    ) {
        ...
        $templateData = str_replace(
            '{{FOREIGN_KEY}}',
            \Str::snake($modelDif) . self::_ID,
            $templateData
        );
        $templateData = str_replace(
            '{{TABLE_FOREIGN_KEY}}',
            $this->serviceGenerator->tableName($modelDif),
            $templateData
        );
    ...
    private function _replaceTemplateRelationshipMTM(
        $model,
        $modelCurrent,
        $templateData
    ) {
    ...
    private function _generateSeederMTM($model, $modelCurrent)
    {
        ...
        $fieldModelCurrent = \Str::snake($modelCurrent) . self::_ID;
        $fileName =
            self::REF_UPPER . $modelCurrent . $model . 'TableSeeder.php';
    ...
    private function replaceTemplate(
        ...
        return $this->serviceGenerator->infy_nl_tab(1, $tabStart) .
            implode(
                $this->serviceGenerator->infy_nl_tab(1, $tabFields),
                $fieldsGenerate
            ) .
    ...
    private function _generateSelect($funcName, $field, $column, $relationship)
    {
        $pathTemplate = 'Forms/';
        if ($relationship === $this->relationship['belongs_to_many']) {
            $formTemplate = $this->serviceGenerator->get_template(
                'selectMTM',
                $pathTemplate,
                'vuejs'
            );
