...
class FormUpdateGenerator extends BaseGenerator
{
    ...
    private function generate($generator, $model, $updateFields)
    {
        ...
        $templateDataReal = $this->importComponent(
            $updateFields,
            $templateDataReal
        );
    ...
    private function generateFieldsRename($renameFields, $templateDataReal)
    {
        ...
        foreach ($renameFields as $index => $rename) {
            ...
            foreach ($dataForms as $form) {
                if (strlen($form) > 0) {
                    ...
                    if ($rename['field_name_old']['field_name'] === $keyForm) {
                        ...
                        if (
                            !in_array($name, $fieldsGenerateDataForm) &&
                            !in_array($keyForm, $arrayChange)
                        ) {
            ...
            $generateSuccess = $this->templateFile(
                $arraySuccess,
                $rename,
                'success'
            );
            ...
            $generateRemove = $this->templateFile(
                $arrayRemove,
                $rename,
                'remove'
            );
            ...
            if (
                ...
                $templateStringify = $this->getHandlerTemplate(
                    'uploadStringify'
                );
                ...
                $templateDataReal = str_replace(
                    $templateStringifyOld,
                    $templateStringifyNew,
                    $templateDataReal
                );
            }
                ...
                if ($index > 0 && $change['id'] === $oldField['id']) {
                    ...
                    if ($change['db_type'] !== $oldField['db_type']) {
                        ...
                        if ($change['db_type'] === $this->dbType['file']) {
                            $templateDataReal = $this->generateFileAndHanler(
                                $change,
                                $templateDataReal
                            );
                        }
                    ...
                    if (
                        $change['default_value'] !== $oldField['default_value']
                    ) {
                        //drop rules
                        if (
                            ...
                            $templateDataReal = $this->dropRules(
                                $change,
                                $model,
                                $templateDataReal
                            );
                        ...
                        if (
                            ...
                            $templateDataReal = $this->generateRule(
                                $change,
                                $model,
                                $templateDataReal
                            );
    ...
    private function generateFieldsUpdate(
        ...
        if (!empty($fieldsGenerateDataForm)) {
            $fieldsGenerateDataForm = array_merge(
                $fieldsGenerateDataForm,
        ...
        foreach ($updateFields as $update) {
            ...
            if ($update['default_value'] === $this->defaultValue['none']) {
                ...
                $templateDataReal = $this->replaceField(
                    $update,
                    $model,
                    $templateDataReal
                );
    ...
    private function importComponent($updateFields, $templateDataReal)
    {
        ...
        foreach ($megerUpdate as $field) {
            if (
                $field['db_type'] === $this->dbType['longtext'] &&
                $flags['import']['long_text']
            ) {
                if (
    ...
    private function generateItems($field, $model)
    {
        ...
        return implode(
            $this->serviceGenerator->infy_nl_tab(1, 3, 2),
            $fieldsGenerate
        );
    }
    ...
    private function generateFields($fields)
    {
        ...
        foreach ($fields as $index => $field) {
            ...
            if (
                ...
                $field['default_value'] === $this->defaultValue['as_define']
            ) {
    ...
    {
        $fieldsGenerate = [];
        $fieldOld = $this->serviceGenerator->modelNameNotPluralFe(
            $fields['field_name_old']['field_name']
        );
        $fieldNew = $this->serviceGenerator->modelNameNotPluralFe(
    ...
    private function templateFile($templates, $fields, $type)
    {
        ...
        foreach ($templates as $template) {
            if (\Str::contains($template, $fieldOld)) {
                ...
                if (
                    \Str::contains($template, $success) ||
                    \Str::contains($template, $removedFile)
                ) {
                    if ($type === 'success') {
                        $fieldsGenerate[] = str_replace(
                            $success,
            ...
            if ($positionEnd) {
                ...
                if ($lengStart) {
                    $replaceStub = substr(
                        $templateRules,
                        $lengStart,
                        $positionEnd - strlen("$fieldName:") - 4
                    );
                }
            }
        }
