...
Class FormUpdateGenerator extends BaseGenerator
{
    ...
    private function generateFieldsRename($renameFields, $templateDataReal)
    {
        ...
        $templateDataForm = $this->serviceGenerator->searchTemplateX($selfDataForm, 1,'},', strlen($selfDataForm) - 1, -strlen($selfDataForm) + 1, $templateDataReal);
        $templateRules = $this->serviceGenerator->searchTemplateX($selfRules, 1, $this->notDelete['rules'], strlen($selfRules) - 1, -strlen($selfRules) + 1, $templateDataReal);
        ...
        foreach($renameFields as $index => $rename) {
            ...
            if($rename['field_name_old']['db_type'] === $this->dbType['file']) {
                $templateDataReal = str_replace($this->serviceGenerator->modelNameNotPluralFe($rename['field_name_old']['field_name']) . 'Temp', $this->serviceGenerator->modelNameNotPluralFe($rename['field_name_new']['field_name']) . 'Temp', $templateDataReal);
            }
            //replace rules
            if(\Str::contains($templateRules, $rename['field_name_old']['field_name'])) {
                $templateRulesTemp = str_replace($this->serviceGenerator->modelNameNotPluralFe($rename['field_name_old']['field_name']), $this->serviceGenerator->modelNameNotPluralFe($rename['field_name_new']['field_name']), $templateRulesTemp);
            }
            //replace function upload file and remove file
            //success
            $successFile = $rename['field_name_old']['field_name'] . self::SUCCESS_FILE;
            $templateSuccessFile = $this->serviceGenerator->searchTemplateX($successFile, 1, "},", -strlen($successFile), strlen($successFile) + 2, $templateDataReal);
...
//            //remove
            $removeFile = $rename['field_name_old']['field_name'] . self::REMOVE_FILE;
            $templateRemoveFile = $this->serviceGenerator->searchTemplateX($removeFile, 1, "},", -strlen($removeFile), strlen($removeFile) + 2, $templateDataReal);
            ...
            if($rename['field_name_old']['db_type'] === $this->dbType['file']) {
                ...
                $templateStringifyOld = str_replace('{{$FIELD$}}', $rename['field_name_old']['field_name'], $templateStringify);
                $templateStringifyNew = str_replace('{{$FIELD$}}', $rename['field_name_new']['field_name'], $templateStringify);
                $templateDataReal = str_replace($templateStringifyOld, $templateStringifyNew, $templateDataReal);
            }
        }
        //form item
        if(!empty($fieldsGenerateDataForm)) {
            $templateDataReal = str_replace("$selfDataForm {".$this->serviceGenerator->infy_nl_tab(1, 0).$templateDataForm."},", "$selfDataForm {".$this->replaceTemplate($fieldsGenerateDataForm, 2, 3, 2)."},", $templateDataReal);
        }
        //rules
                ...
                if($index > 0 && $change['id'] === $oldField['id']) {
                    ...
                    if($change['db_type'] !== $oldField['db_type']) {
                        ...
                        if($oldField['db_type'] === $this->dbType['file']) {
                            $templateDataReal = str_replace($this->serviceGenerator->modelNameNotPluralFe($change['field_name']) . 'Temp: [],', "", $templateDataReal);
                            //drop file success
                            $successFile = $change['field_name'] . self::SUCCESS_FILE;
                            $templateSuccessFile = $this->serviceGenerator->searchTemplateX($successFile, 1, "},", -strlen($successFile), strlen($successFile) + 2, $templateDataReal);
                            if($templateSuccessFile) {
                                $templateDataReal = str_replace($templateSuccessFile, "", $templateDataReal);
                            }
                            //drop file remove
                            $removeFile = $change['field_name'] . self::REMOVE_FILE;
                            $templateRemoveFile = $this->serviceGenerator->searchTemplateX($removeFile, 1, "},", -strlen($removeFile), strlen($removeFile) + 2, $templateDataReal);
                            ...
                            //drop json stringify
                            $templateStringify = $this->getHandlerTemplate('uploadStringify');
                            $templateStringifyNew = str_replace('{{$FIELD$}}', $change['field_name'], $templateStringify);
                            $templateDataReal = str_replace($templateStringifyNew, "", $templateDataReal);
                        }
                        // add file
    ...
    private function generateFieldsDrop($dropFields, $model, $templateDataReal)
    {
        ...
        foreach($dropFields as $index => $drop) {
            ...
            if($drop['db_type'] === $this->dbType['file']) {
                // drop file temp
                $templateDataReal = str_replace($this->serviceGenerator->modelNameNotPluralFe($drop['field_name']) . 'Temp: [],', '', $templateDataReal);
                //drop file Drop zone
                $templateDataReal = str_replace($this->serviceGenerator->modelNameNotPluralFe($drop['field_name']) . 'Temp: [],', "", $templateDataReal);
                //drop file success
                $successFile = $drop['field_name'] . self::SUCCESS_FILE;
                $templateSuccessFile = $this->serviceGenerator->searchTemplateX($successFile, 1, "},", -strlen($successFile), strlen($successFile) + 2, $templateDataReal);
                if($templateSuccessFile) {
                    $templateDataReal = str_replace($templateSuccessFile, "", $templateDataReal);
                }
                //drop file remove
                $removeFile = $drop['field_name'] . self::REMOVE_FILE;
                $templateRemoveFile = $this->serviceGenerator->searchTemplateX($removeFile, 1, "},", -strlen($removeFile), strlen($removeFile) + 2, $templateDataReal);
                if($templateRemoveFile) {
                    $templateDataReal = str_replace($templateRemoveFile, "", $templateDataReal);
                }
                //drop json stringify
                $templateStringify = $this->getHandlerTemplate('uploadStringify');
                $templateStringifyNew = str_replace('{{$FIELD$}}', $drop['field_name'], $templateStringify);
                $templateDataReal = str_replace($templateStringifyNew, "", $templateDataReal);
            }
            //drop rules
    ...
    private function generateFields($fields)
    {
        ...
        foreach($fields as $index => $field) {
            ...
            } else if($field['default_value'] === $this->defaultValue['as_define']) {
    ...
    private function templateForm($templates, $fields)
    {
        ...
        foreach($templates as $template) {
            if(\Str::contains($template, $fields['field_name_old']['field_name'])) {
                $id = 'id="'.$this->serviceGenerator->modelNameNotPluralFe($fields['field_name_old']['field_name']). 'VueDropzone"';
                $success = '@success="'.$this->serviceGenerator->modelNameNotPluralFe($fields['field_name_old']['field_name']). 'DropzoneS"';
                $removedFile = '@removedFile="'.$this->serviceGenerator->modelNameNotPluralFe($fields['field_name_old']['field_name']). 'DropzoneR"';
                if($template === $id || $template === $success || $template === $removedFile) {
                    $fieldsGenerate[] = str_replace($this->serviceGenerator->modelNameNotPluralFe($fields['field_name_old']['field_name']), $this->serviceGenerator->modelNameNotPluralFe($fields['field_name_new']['field_name']), $template);
                    continue;
                }
    ...
    private function templateFile($templates, $fields, $type)
    {
        ...
        foreach($templates as $template) {
            if(\Str::contains($template, $fields['field_name_old']['field_name'])) {
                $success = $this->serviceGenerator->modelNameNotPluralFe($fields['field_name_old']['field_name']). $dropzoneS;
                $removedFile = $this->serviceGenerator->modelNameNotPluralFe($fields['field_name_old']['field_name']). $dropzoneR;
                //replace function name
                if(\Str::contains($template, $success) || \Str::contains($template, $removedFile)) {
                    if($type === 'success') {
                        $fieldsGenerate[] = str_replace($success, $this->serviceGenerator->modelNameNotPluralFe($fields['field_name_new']['field_name']) . $dropzoneS, $template);
                    } else if($type === 'remove') {
                        $fieldsGenerate[] = str_replace($removedFile, $this->serviceGenerator->modelNameNotPluralFe($fields['field_name_new']['field_name']) . $dropzoneR, $template);
                    }
                    continue;
                }
                //replace file temp
...
//                }
    ...
    {
        $ruleStub = '{ required: true, message: this.$t(\'validation.required\', { attribute: {{$ATTRIBUTE_FIELD$}} }), trigger: [\'change\', \'blur\'] },';
        $attribute = 'this.$t(\'table.' . $this->serviceGenerator->tableNameNotPlural($model['name']) . '.' . $field['field_name'] . "')";
        $replaceStub = str_replace('{{$ATTRIBUTE_FIELD$}}', $attribute, $ruleStub);
