...
Class PageUpdateGenerator extends BaseGenerator
{
	/** @var $service */
	...
	public function __construct($model, $updateFields)
	{
		...
		$this->notDelete = config('generator.not_delete.vuejs.page');
			...
			foreach($classes as $class) {
				if(strlen($class) > 0) {
					...
					if($update['field_name'] === $keyClass) {
						if($update['show']) {
							switch ($update['db_type']) {
								...
								case $this->dbType['enum']:
									$fieldsGenerateClasses[] = "'".$update['field_name']."': 'has-text-centered',";
									break;
							}
						}
					} else {
						...
						$name = "'".$update['field_name']."': 'has-text-centered',";
	...
	private function generateFieldsUpdate($updateFields, $model, $templateDataReal)
	{
        ...
        $createClasses = "'".self::CREATE_AT."': 'has-text-centered',";
        foreach($classes as $class) {
            if (strlen($class) > 0) {
                $class = trim($class);
                list($keyClass, $valClass) = explode(': ', $class);
